...
def ReportError(callbackUrl, signature):
    ...
    r = requests.post(callbackUrl, data={'signature': signature, 'returntype': 'failed', 'returnmsg': message})
    raise
...
def CheckExperiment(callbackUrl, signature, modelUrl, protocolUrl):
    ...
    try:
        ...
        if missing_terms:
            ...
            r = requests.post(callbackUrl, data={'signature': signature, 'returntype': 'inappropriate', 'returnmsg': message})
